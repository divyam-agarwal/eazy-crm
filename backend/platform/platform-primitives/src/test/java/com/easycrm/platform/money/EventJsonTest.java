...
class EventJsonTest {
    ...
    void serialisesBigDecimalAsAString() {
        ...
        String json =
                EventJson.mapper().writeValueAsString(new Payload(new BigDecimal("1180.00"), Instant.EPOCH, null));
    ...
    void keepsNullFieldsRatherThanOmittingThem() {
        ...
        String json = EventJson.mapper().writeValueAsString(new Payload(BigDecimal.ONE, Instant.EPOCH, null));
    ...
    void ignoresUnknownPropertiesOnRead() {
        ...
        Payload p = EventJson.mapper()
                .readValue(
                        "{\"grandTotal\":\"5.00\",\"occurredAt\":\"2026-08-27T09:15:30Z\","
                                + "\"note\":null,\"fieldFromANewerProducer\":42}",
                        Payload.class);
