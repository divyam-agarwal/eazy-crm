...
class InvitationListRevokeTest extends IntegrationTest {
    ...
    void salesExecCanNeitherListNorRevoke() throws Exception {
        ...
        mvc.perform(get("/api/v1/invitations").header("Authorization", "Bearer " + exec))
                .andExpect(status().isForbidden());
        mvc.perform(delete("/api/v1/invitations/" + id).header("Authorization", "Bearer " + exec))
                .andExpect(status().isForbidden());
    }
    ...
    void anotherTenantsOwnerSeesNothingAndCannotRevoke() throws Exception {
        ...
        mvc.perform(get("/api/v1/invitations").header("Authorization", "Bearer " + b.token()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(0));

        mvc.perform(delete("/api/v1/invitations/" + id).header("Authorization", "Bearer " + b.token()))
                .andExpect(status().isNotFound());
    }
}
