...
class NestedVisibilityTest extends IntegrationTest {
    ...
    void seed() throws Exception {
        ...
        contactUnderB = UUID.fromString(JsonPath.read(mvc.perform(
                post("/api/v1/customers/" + customerB + "/contacts")
                .header(AUTH, bearer(ownerToken)).contentType(MediaType.APPLICATION_JSON).content("""
                    {"name":"Ramesh","isPrimary":true}"""))
            .andReturn().getResponse().getContentAsString(), "$.id"));

        String pId = JsonPath.read(mvc.perform(post("/api/v1/products").header(AUTH, bearer(ownerToken))
        ...
        quoteUnderB = UUID.fromString(JsonPath.read(mvc.perform(post("/api/v1/quotations")
                .header(AUTH, bearer(ownerToken)).contentType(MediaType.APPLICATION_JSON).content(
        ...
        String publicUrl = JsonPath.read(mvc.perform(
                post("/api/v1/quotations/" + quoteUnderB + "/share").header(AUTH, bearer(ownerToken)))
            .andExpect(status().isOk())
    ...
    void execCannotUpdateAContactUnderAnInvisibleCustomer() throws Exception {
        mvc.perform(put("/api/v1/customers/" + customerB + "/contacts/" + contactUnderB)
                .header(AUTH, bearer(execAToken)).contentType(MediaType.APPLICATION_JSON).content("""
                    {"name":"Ramesh Updated"}"""))
            .andExpect(status().isNotFound());
    }
