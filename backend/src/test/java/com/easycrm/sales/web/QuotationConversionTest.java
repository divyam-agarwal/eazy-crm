...
class QuotationConversionTest extends IntegrationTest {
    ...
    private String seedEnquiry(String auth, String phone) throws Exception {
        ...
        String eBody = mvc.perform(post("/api/v1/enquiries")
                        .header("Authorization", auth)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andExpect(status().isCreated())
    ...
    void raisingQuoteFromEnquiryConvertsItAndStampsTheQuotation() throws Exception {
        ...
        String body = """
            {"customerId":"%s","enquiryId":"%s","items":[{"productId":"%s","qty":"2"}]}""".formatted(ids[0], enquiryId, ids[1]);
    ...
    void unknownEnquiryIdReturns404() throws Exception {
        ...
        String[] ids = seed(auth, "27");
        String body = """
            {"customerId":"%s","enquiryId":"%s","items":[{"productId":"%s","qty":"1"}]}""".formatted(ids[0], UUID.randomUUID(), ids[1]);
    ...
    void crossTenantEnquiryIdReturns404AndLeavesItUntouched() throws Exception {
        ...
        String body = """
            {"customerId":"%s","enquiryId":"%s","items":[{"productId":"%s","qty":"1"}]}""".formatted(idsB[0], enquiryId, idsB[1]);
    ...
    void quotingAnAlreadyTerminalEnquiryReturns422() throws Exception {
        ...
        mvc.perform(post("/api/v1/enquiries/" + enquiryId + "/lose")
                        .header("Authorization", auth)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"lostReason\":\"bought elsewhere\"}"))
                .andExpect(status().isOk());

        String body = """
            {"customerId":"%s","enquiryId":"%s","items":[{"productId":"%s","qty":"1"}]}""".formatted(ids[0], enquiryId, ids[1]);
        mvc.perform(post("/api/v1/quotations")
                        .header("Authorization", auth)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
    ...
    void failedQuoteBuildRollsBackTheConversion() throws Exception {
        ...
        String body = """
            {"customerId":"%s","enquiryId":"%s","items":[{"productId":"%s","qty":"0"}]}""".formatted(ids[0], enquiryId, ids[1]);
        mvc.perform(post("/api/v1/quotations")
                        .header("Authorization", auth)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
