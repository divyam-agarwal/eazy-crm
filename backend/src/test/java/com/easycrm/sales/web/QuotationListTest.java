...
class QuotationListTest extends IntegrationTest {
    ...
    private String createQuotation(String auth) throws Exception {
        ...
        String pBody = mvc.perform(post("/api/v1/products")
                ...
                .getContentAsString();
        String body = """
            {"customerId":"%s","items":[{"productId":"%s","qty":"1"}]}""".formatted((String) JsonPath.read(cBody, "$.id"), (String) JsonPath.read(pBody, "$.id"));
        return mvc.perform(post("/api/v1/quotations")
                        .header("Authorization", auth)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(body))
                .andReturn()
    ...
    private String quotation(String auth, String customerId) throws Exception {
        ...
        String pBody = mvc.perform(post("/api/v1/products")
                ...
                .getContentAsString();
        String body = """
            {"customerId":"%s","items":[{"productId":"%s","qty":"1"}]}""".formatted(customerId, (String) JsonPath.read(pBody, "$.id"));
        return JsonPath.read(
    ...
    void statusAndCustomerFiltersApplyTogether() throws Exception {
        ...
        mvc.perform(get("/api/v1/quotations?status=SENT&customerId=" + wanted).header("Authorization", auth))
