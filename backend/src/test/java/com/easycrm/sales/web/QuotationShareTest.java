...
class QuotationShareTest extends IntegrationTest {
    ...
    private String customer(String auth, String state) throws Exception {
        return JsonPath.read(
                mvc.perform(post("/api/v1/customers")
                                ...
                                .content("""
                    {"businessName":"Bharat Industries","stateCode":"%s","source":"MANUAL"}""".formatted(state)))
                        .andReturn()
                        .getResponse()
                        .getContentAsString(),
                "$.id");
    ...
    private void addContact(String auth, String customerId, String phone, String whatsapp) throws Exception {
        mvc.perform(post("/api/v1/customers/" + customerId + "/contacts")
                .header("Authorization", auth)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                {"name":"Ramesh","phone":%s,"whatsappNumber":%s,"isPrimary":true}""".formatted(
                                phone == null ? "null" : "\"" + phone + "\"",
                                whatsapp == null ? "null" : "\"" + whatsapp + "\"")));
    }

    private void addContactWithPrimary(String auth, String customerId, String whatsapp, boolean isPrimary)
            throws Exception {
        mvc.perform(post("/api/v1/customers/" + customerId + "/contacts")
                .header("Authorization", auth)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                {"name":"Contact","whatsappNumber":"%s","isPrimary":%s}""".formatted(whatsapp, isPrimary)));
    }
