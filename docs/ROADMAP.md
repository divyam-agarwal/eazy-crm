...
**2026-09-20 (later) — latest.** **F0b is MERGED and PUSHED.** `main` and `origin/main` are both at
`6d7b970` (fast-forward, 38 commits, `389f23b..6d7b970`); the `f0b-frontend` branch and its worktree are
deleted. Full verification was re-run on the merged tree, not inherited from the branch: lint, typecheck,
...
all six budgeted routes under 200 KB with headroom (`/signup` 176.2 KB tightest).
`docs/superpowers/HANDOFF.md`'s top section has full detail, the manual walkthrough results (including
the answered R64 Devanagari-clipping question — no clipping found), and notes for F1. The 107 controller
rulings made during execution are preserved at
[`superpowers/2026-09-20-f0b-ruling-record.md`](superpowers/2026-09-20-f0b-ruling-record.md), because the
SDD workspace they were written in is git-ignored and was deleted with the worktree.
...
| Tests | **688 (backend), 0 failures** on `main` — merged and pushed with F0b at `6d7b970`; was 677 before it (F0a added 51, 2026-09-16; Wave 1.6 added 22). Plus **307 frontend Vitest tests** and **11 Playwright E2E**. **On the `worktree-f1a-backend-prep` branch (F1a, 2026-09-24): 751 backend tests, 0 failures — NOT YET MERGED, NOT YET PUSHED** |
...
| Frontend | **F0b (2026-09-20, merged and pushed at `6d7b970`, CI green):** React + TypeScript SPA — login, signup, `/invite/:token` (anonymous, signed-in, invalid), app shell. 307 Vitest tests / 33 files, coverage floor 97/92/96/98, 11/11 Playwright E2E against two real backends, six routes each under the 200 KB gzip budget. See `docs/superpowers/HANDOFF.md` top section |
...
| CI | GitHub Actions, `push: [main]` and `pull_request`, JDK 25, Testcontainers. **Five jobs since F0b:** `check`, `supply-chain` (gitleaks + actionlint + squawk, blocking), `dependency-check` (OWASP, reports rather than gates), **`frontend`** (lint, typecheck, Vitest + coverage floor, generated-client drift, JS budget) and **`e2e`** (Playwright against two real backends). First green run of all five: `35493017772`, 2026-09-20. **Note the trigger: a feature-branch push fires NOTHING — CI on a branch requires a PR, which is why this repo's gates are all post-merge** |
...
**password reset** · **outbox, SNS/SQS, any second service**. (Frontend F0 is merged and pushed —
see §1.1; F1–F3 master data / wedge / daily work are still ahead. `/invite/{token}` now has a real
page, built in F0b.)
...
| **H5** | **Corrected 2026-09-24 (F1a Task 10):** the `assigned_to = :me OR assigned_to IS NULL` half is STALE — `V33__assigned_to_indexes.sql` already shipped `idx_customer_assigned` and `idx_enquiry_assigned`. What remains true: no index supports a status-only order-list filter (`sales_order` carries only `(tenant_id, id)` and `(tenant_id, customer_id)`) | HANDOFF §8 |
...
| Track | Have | Left |
|---|---|---|
| **Application** | Wedge end-to-end, multi-user, activity/follow-up, auto-expiry, PDF + share, **buyer snapshot (H1 closed)**, 688 tests, pushed. **F1a (master data backend prep) BUILT on `worktree-f1a-backend-prep`, 751 tests, review-clean — NOT YET MERGED, NOT YET PUSHED (2026-09-24)** | F1b (frontend for master data) next. Cursor pagination, `SALES_MANAGER` tier (H6), password reset, self-service profile |
| **Frontend** | **F0 DONE, merged and pushed at `6d7b970`, CI green (2026-09-20)**: login, signup, `/invite/:token` (all three states), app shell, session lifecycle with cross-tab logout/refresh handling, 307 frontend tests, 11/11 E2E, coverage floor, CI wired (`frontend` + `e2e` jobs) | **F1 (master data) → F2 (the wedge) → F3 (daily work + team)** |
...
| **4** | **Frontend** | The biggest product step and the only one a backend slice cannot finish. The contract is ready and guarded; building now means the contract shapes the client rather than the reverse. **Taken NEXT, ahead of H7, at the owner's direction (2026-09-14).** Decomposed into F0–F3 (D-d settled, see Phase 1). **F0a (backend auth prep) DONE, merged and pushed (2026-09-16) — [plan](superpowers/plans/2026-09-14-f0a-backend-auth-prep.md); F0b (the frontend itself) BUILT, MERGED AND PUSHED at `6d7b970` with CI green (2026-09-20) — [plan](superpowers/plans/2026-09-16-f0b-frontend-foundation.md); F1a (master data backend prep) BUILT on `worktree-f1a-backend-prep`, review-clean, 751 tests — NOT YET MERGED, NOT YET PUSHED (merge/push is the owner's call, 2026-09-24); next: F1b (master data frontend), generating its TypeScript client from the regenerated contract.** | **L** | DNS provider (rest of D-g) before any public link ships |
...
1. **Item 4, the frontend — F1b next. F0b is MERGED AND PUSHED at `6d7b970`, CI green (2026-09-20).
   F1a (master data backend prep) is BUILT and review-clean on `worktree-f1a-backend-prep`
   (751 tests) but NOT YET MERGED and NOT YET PUSHED — both are the owner's call (2026-09-24).**
   ([`plans/2026-09-16-f0b-frontend-foundation.md`](superpowers/plans/2026-09-16-f0b-frontend-foundation.md),
   [`plans/2026-09-24-f1a-backend-master-data-prep.md`](superpowers/plans/2026-09-24-f1a-backend-master-data-prep.md)).
   D-d (the decomposition) is settled: F0 foundation + auth + invite page → **F1 master data** (backend
   prep done as F1a; **F1b, the frontend, next**) → F2 the wedge → F3 daily work + team, with the import
   wizard and owner analytics deferred because no backend exists for them. HANDOFF.md's top section has
   the full F1a verification record and the inputs F1b needs to inherit.
