...
**Last updated:** 2026-07-27 (sales hardening — optimistic-lock→409 + quote-uniqueness backstop — done on branch `sales-hardening`, pending merge; enquiry→quotation conversion merged to `main` as `06e6014`; enquiry slice merged as `a68035d`)
...
16. **`plans/2026-07-27-sales-hardening.md`** — sales hardening implementation plan (**DONE on branch `sales-hardening`, pending merge**).
...
- **Branch:** `sales-hardening` (off `main` @ `72691d6`) — optimistic-lock→409 + quote-uniqueness backstop done, pending final review + merge. `main` itself is clean (conversion merged).
...
**The sales-hardening slice is DONE on branch `sales-hardening`, pending final review + merge.** 3 tasks landed and reviewed (optimistic-lock→409 handler + tests; `UNIQUE(tenant_id, enquiry_id)` migration/entity + tests; docs), each task-review clean. It closes the two Minors that the enquiry→quotation conversion whole-branch review consciously deferred (both now struck from the deferred list below). Clean-build total is **166 tests**, all green. Prior slice (enquiry→quotation conversion) is merged to `main` (`06e6014`). Next step is to pick the next chunk with the user (see §8).
...
- ~~**Optimistic-lock → 409 (codebase-wide)**~~ — **DONE** (sales-hardening slice, pending merge). A global `@ExceptionHandler(OptimisticLockingFailureException.class)` → 409 now maps a lost-update race (concurrent `accept` challenge #21, or convert-at-create challenge #25) to 409 instead of 500 — a sibling of the challenge #15 `DataIntegrityViolation` backstop on the disjoint transient/concurrency subtree. Challenge #26.
- ~~**Structural backstop for one-quote-per-enquiry**~~ — **DONE** (sales-hardening slice, pending merge). `UNIQUE(tenant_id, enquiry_id)` on `quotation` (migration `V22` + entity `@Table`; Postgres NULLs distinct, so enquiry-less quotes coexist) makes the one-quote-per-enquiry invariant structural. A guard-bypassed/raced second insert now routes through the challenge #15 handler → 409. Challenge #26.
