...
- **Optimistic-lock → 409 (codebase-wide)** — a lost-update race (concurrent quotation `accept`, challenge #21; or concurrent convert-at-create, challenge #25) is caught by `@Version` so integrity holds, but the losing request currently surfaces as **HTTP 500**: `ObjectOptimisticLockingFailureException` extends `ConcurrencyFailureException`, missing the challenge #15 `DataIntegrityViolation`→409 backstop. Deferred to a dedicated hardening slice: one global `@ExceptionHandler(OptimisticLockingFailureException.class)` → 409 + a race test. (Reviewed and consciously deferred during the conversion slice.)
- **Structural backstop for one-quote-per-enquiry** — one enquiry maps to at most one quotation, but this rests only on the entity terminal guard + `@Version` (procedural), with no DB constraint. A `UNIQUE(tenant_id, enquiry_id)` on `quotation` (Postgres allows multiple NULLs, so enquiry-less quotes are unaffected) would make it structural, mirroring the enquiry dedupe partial index (challenge #23). Spec deliberately framed it as emergent; deferred.
