...
| Annotation | Origin | Purpose | Composed of / inherits |
|---|---|---|---|
| `@Query` | `org.springframework.data.jpa.repository` | Supplies an explicit JPQL query for a repository method, overriding derived-query-by-method-name (`DocumentCounterRepository.findForUpdate`, keyed on `docType`+`fy` — the tenant filter still applies via `@TenantId`, see challenge #8). Also used with `nativeQuery = true` for `RefreshTokenRepository.revokeIfLive` — a hand-written conditional `UPDATE ... WHERE revoked_at IS NULL` that turns a lost optimistic-lock race into a clean zero-row match instead of `ObjectOptimisticLockingFailureException` (challenge #80). | — |
| `@Param` | `org.springframework.data.repository.query` | Binds a method parameter to a named `:param` placeholder in an `@Query` string. | — |
| `@Modifying` | `org.springframework.data.jpa.repository` | Marks a `@Query` method as an UPDATE/DELETE rather than a SELECT, so Spring Data executes it via `executeUpdate()` and returns the affected row count instead of trying to map a result set. `flushAutomatically = true` (`RefreshTokenRepository.revokeIfLive`) flushes the persistence context's pending changes before running the native UPDATE, so an INSERT queued earlier in the same transaction (the rotation's successor row) is visible for the UPDATE's own `replaced_by_id` foreign reference. A native `@Modifying` query bypasses `@Version` and Spring Data auditing entirely — both must be set by hand in the query (challenge #80). | — |
| `@Lock` | `org.springframework.data.jpa.repository` | Forces a `LockModeType` on the query's execution; `PESSIMISTIC_WRITE` compiles to `SELECT … FOR UPDATE`, serializing concurrent readers of the same row (`DocumentCounterRepository.findForUpdate`, so concurrent quote sends within a tenant/FY get gapless numbers — must run inside the caller's tenant-bound `@Transactional`, or the lock is never acquired inside a real transaction). Second use site: `TenantRepository.findForUpdate`, which locks the tenant row at the top of every member-admin write so two concurrent role changes can't both pass the last-active-owner count check — the row being locked is not the row being written, it exists purely to serialize the invariant check (challenge #65). | — |
...
| `@Operation` | `io.swagger.v3.oas.annotations` | Operation-level metadata for one handler method. Used here only for `description`, on `AuthController.refresh` and `logout`, to say in the contract that they authenticate with the httpOnly `easycrm_rt` cookie and the `X-EasyCRM-Client: web` header rather than a bearer token (F0a final review). A method-level `@ApiResponse` on the same method overrides the advice-level one for that status code only, which is how those two routes replace the generic bearer-token 401/403 descriptions. | — |
| `@Parameter` / `@Parameters` | `io.swagger.v3.oas.annotations` | Documents a parameter. At method level inside `@Parameters`, with `in = ParameterIn.HEADER`, it declares a header the handler reads **without** binding a method argument — used for the required `X-EasyCRM-Client` header on refresh and logout, whose check stays explicit code because `@RequestHeader` would answer 400 instead of 403 (F0a final review). Documentation only. | `@Parameters` is the container for `@Parameter` |
...
| Annotation | Origin | Purpose | Composed of / inherits |
|---|---|---|---|
| `@ActiveProfiles` | `org.springframework.test.context` | Activates the named Spring profiles for the test's `ApplicationContext`. `DevApiDocsSecurityConfigTest` is the **only** class in this suite that uses it (`"dev"`), because it is the only place `DevApiDocsSecurityConfig` — a `@Profile("dev")` `@Order(0)` `SecurityFilterChain` — exists at all; without it no test could assert anything about that bean. It is part of the context cache key, so each distinct value costs one additional Spring context: deliberately one class, not several (openapi-contract review wave). | — |
