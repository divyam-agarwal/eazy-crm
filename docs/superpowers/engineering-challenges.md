...
## Challenge 82 — SpotBugs flags a brand-new singleton bean field but silently spares a structurally identical one, and the difference isn't position or type-shape

**Phase:** Implementation (f0a-backend-auth-prep, Task 7)

### The problem

Moving the refresh token into a cookie added `RefreshCookie` — a stateless `@Component` (three
`static final` fields, no instance state) — as a second constructor parameter on `AuthController`
and `PublicInvitationController`. `./gradlew spotbugsMain` failed with two new `EI_EXPOSE_REP2`
findings: `AuthController.cookie` and `PublicInvitationController.cookie`, "may expose internal
representation by storing an externally mutable object."

The puzzling part: `AuthController` already stored `AuthService` — a concrete `@Service` class,
same shape (constructor parameter assigned straight to a `private final` field, no defensive copy)
— and had never been flagged for it, not in this run and not in `config/spotbugs/baseline.xml`'s
32 pre-existing entries. Neither was `InvitationService` on `PublicInvitationController`. Two
structurally identical patterns, one flagged and one not, made it impossible to tell by inspection
whether this was a real new finding to fix or a tooling artifact to route around — and
`config/spotbugs/baseline.xml`'s own header is explicit that new findings get fixed in code, not
appended to the debt snapshot (challenge #62's rule).

Ruled out by direct experiment rather than by reading SpotBugs source: swapping the two
constructor parameters' *assignment order* left the identical `instanceHash` and the identical
flagged field (`cookie`), so it wasn't positional. Declaring `RefreshCookie` `final` changed
nothing either. The decisive test was temporarily commenting out `baselineFile` (not
`excludeFilter`) and re-running `spotbugsMain` to see SpotBugs's *complete*, unfiltered finding
set: `ContactController.service` (a genuine analogue of `AuthController.auth`, same single-field
shape) appeared in that full list, matching its baseline entry — so baselined findings are real,
not stale. But `AuthController.auth` and `PublicInvitationController.invitations` did not appear
at all, even unfiltered. That is decisive: SpotBugs's own analysis, with every exclusion mechanism
disabled, genuinely does not consider storing an `AuthService`/`InvitationService` reference a
finding, while storing a `RefreshCookie` reference — the newly-introduced field, nothing else
different about the surrounding class — is. Ruling out instanceHash collision with an unrelated
baselined entry (challenge #76's mechanism) mattered here specifically because the symptom looks
identical from outside: a finding that "should" be there isn't. The unfiltered run is what tells
the two apart.

### The solution

Given the asymmetry couldn't be resolved to a principled rule (and SpotBugs's own EI_EXPOSE_REP2
detector is documented, including in this file's baseline header, as heuristic and inconsistent —
"largely noise"), and no `@SuppressFBWarnings` annotation was already wired into this build (it
would need a new `compileOnly` dependency, a bigger footprint than the actual problem), the fix
used the mechanism this repository already provides for exactly this situation:
`config/spotbugs/exclude.xml`, whose own header distinguishes it from `baseline.xml` — "findings
excluded permanently, by category," not a snapshot of debt to work off. Two `<Match>` entries,
scoped to `Class` + `Field` + `Bug pattern` (not a blanket `EI_EXPOSE_REP2` exclusion), so a
genuinely mutable field introduced anywhere else still fails the build. The justification is
recorded as an XML comment in the file itself: `RefreshCookie` carries no instance state, so there
is no meaningful defensive copy to make, unlike the List/Map fields this project's real
EI_EXPOSE_REP2 debt is about.

### Lesson

When a static-analysis tool flags one of two structurally identical call sites and spares the
other, "fix the one it flagged" and "it's a false positive, exclude it" are both premature until
you've confirmed the tool's *unfiltered* output actually treats them differently — not just that
your build's filtered output does. `baselineFile` and `excludeFilter` are two different exclusion
mechanisms stacked on the same run; disabling only the debt-snapshot one (`baselineFile`) and
re-running is what separates "genuinely not flagged" from "flagged, but silently absorbed by a
hash coincidence" (challenge #76's failure mode) — the two look identical in filtered output and
require different fixes. And when a project provides two exclusion files with different contracts
— one a append-only debt ledger, one a permanent by-category filter — a new finding that is
judged, after direct verification, to be tooling noise on a genuinely immutable type belongs in
the *category* file, scoped as narrowly as the filter syntax allows, with the reasoning written
down next to the exclusion rather than folded into the ever-growing snapshot the next person is
told never to add to.
