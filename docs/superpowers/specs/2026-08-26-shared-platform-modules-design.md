...
| 5 | `platform-outbox` | **revise** (504 lines exist) | Two revisions remain: declare the `platform-primitives` dependency; align with the sealed API (its `runAs(new TenantPrincipal(…, "SYSTEM"))` becomes `SystemPrincipal`). **The third is discharged** — LLD #4's CD2 rules that there is no `TenantAwareSqsListener` to align with: `platform-tenancy` owns the binding primitive and `IdempotentConsumer` composes it |
