...
| P3 | **`TenantContext` sealed: `runAs` public, `set`/`clear` package-private** | Leave as is; migrate to `ScopedValue` (JEP 506) | The compiler enforces it — a service that calls `set()` no longer compiles, so the `finally` is written once in `platform-tenancy` instead of at **137 call sites** (122 in tests across 54 files, 15 in main — counted at `80e74a3` while writing LLD #4). `ScopedValue` gives a stronger guarantee but costs that same migration *plus* a primitive rewrite, for a failure mode P3 already closes |
...
├── persistence/
│   ├── BaseEntity                   time-sortable id, @Version, timestamps  (NOT UUIDv7 — PF16)
...
**Both of LLD #3's constraints are discharged** in
[`../../architecture/2026-08-27-platform-tenancy-lld.md`](../../architecture/2026-08-27-platform-tenancy-lld.md):
SR4 becomes CR6 (the filter catches `InvalidTokenException` only and lets `KeyUnavailableException`
reach a 503), and SF2 is closed structurally by CD3 — `TenantPrincipal` becomes a **sealed interface**
whose implementations *are* the provenance, so `SystemPrincipal` has no `role()` for a role check to
reach and `userId` exists only where it is real.
...
**Contents.** `Metric`, `@RequiresEntitlement(Metric)`, **`@NotMetered(reason)`**, the
`UsageCounter` SPI, the guard, its own `@RestControllerAdvice` for the 402 (ED3 — `platform-web` may
not name this module), and the ArchUnit rule that fails the build for a create endpoint carrying
neither annotation.
...
| 5 | `platform-outbox` | **revised** — [`../../architecture/2026-08-19-outbox-lld.md`](../../architecture/2026-08-19-outbox-lld.md) Part 0 | Five revisions applied in place, not a rewrite: the `platform-primitives`/`platform-tenancy` dependencies declared (TB3's structural fix); `EventJson` replaces the mapper; `SystemPrincipal` replaces the string provenance; **`relay_app` loses `BYPASSRLS`** for two role-scoped policies on the outbox table alone (OF6); and ShedLock finally gets a table (OF7). Two new findings feed back — OF8 amends LLD #4's CD1, OF9 extends its CF8 |
| 6 | `platform-entitlement` | **written** — [`../../architecture/2026-08-27-platform-entitlement-lld.md`](../../architecture/2026-08-27-platform-entitlement-lld.md) | Settled: `@RequiresEntitlement` xor `@NotMetered(reason)` on every create, so the rule need not know what is metered — only that someone decided in writing (ED2, the `@GlobalTable` idiom reused); the guard is advisory under concurrency and that is recorded as a property of a *commercial* limit (ED1); the 402 advice ships here because `platform-web` may not name this module (ED3). Found PF18–PF19. **Still built in sub-project 10** — only the shape is fixed now, because it constrains every controller written before then |
...
| **PF6** | P5's rule — a mechanism used by one service is not a platform mechanism — has **no automated enforcement**. The D12 ArchUnit rule passes happily for `PdfEngine` | Accepted. It is a review rule. An ArchUnit rule counting consumers would need the full service graph at test time. **It now has company:** `@GlobalTable(reason)` (LLD #4, CR1) and `@NotMetered(reason)` (LLD #6, ED2) both force a justification to be *written* without being able to check that it is *good*. Three review rules of one shape — the pattern is deliberate, not three separate concessions |
...
| **PF14** | **Row-Level Security is `ENABLE`d on all fourteen tables and `FORCE`d on none.** In Postgres a table's owner is exempt from its own policies unless the table is forced, so layer 3 currently rests entirely on the application connecting as the non-owner `easycrm_app`. That is deliberate and safe with one deployment; with **five services holding five sets of Terraform-issued credentials**, one service issued the owner role loses RLS silently — no error, no log, no failing test, because the harness wires both roles correctly by construction | `FORCE ROW LEVEL SECURITY` on every tenant-scoped table, making isolation a property of the schema rather than of who connects. Gated on whether Flyway (which owns the tables) then needs `BYPASSRLS` — LLD #4 Appendix B item 2. **The most serious finding in any of the four LLDs** |
| **PF15** | **ArchUnit guards layer 2; nothing guards layer 3.** A new table that declares `@TenantId` and omits its two RLS lines passes the entire suite — Hibernate filters it, so behaviour is correct — and ships with one of the four isolation layers simply absent. Relatedly, the global-table allowlist is three FQNs spanning two future services inside a single test class, so the split dissolves the one-list-reviewed-together property that justified it | A `pg_policy`/`pg_class` integration test asserting every `TenantScopedEntity` table is RLS-enabled, forced and carries `tenant_isolation` (LLD #4 CR3); and `@GlobalTable(reason = "…")` on the entity replacing the FQN list (CR1), so the justification is reviewed next to the thing it justifies |
| **PF16** | **Three of the six entry points have no consumer at all** — there is no `@Scheduled`, no `@Async` and no SQS listener anywhere in the codebase, and `AsyncConfig` wires a decorator into an executor nothing submits to. P1 was written before this was visible, and it sits against P5. Separately, §2.4's contents list claims `BaseEntity` has a **UUIDv7** id; it uses Hibernate's `@UuidGenerator(style = TIME)`, a different format, while the real `UuidV7` class serves `Tenant` alone — two id formats, neither described correctly | CD1 — all six specified now, three built now, three built with their first consumer; ownership under P1 unchanged, only the schedule. The id description corrected in §2.4 above; the formats deliberately **not** unified, since both are time-sortable and a migration would rewrite every primary key to buy nothing (LLD #4 CF1) |
| **PF18** | **"Keep it opaque" is load-bearing, not stylistic — and it has now prevented three cycles.** LLD #3 kept `role` an opaque `String` because `Role` lives in `identity-svc`; LLD #4 kept `Entitlements` opaque because `Metric` lives in `platform-entitlement`; `VerifiedClaims` carries claims as data because `platform-security` may name nothing at all. Each was written as a local judgement call; together they are one rule | **A lower module carries the value; an upper module supplies the meaning.** This is D12's "mechanisms, never meanings" restated as a dependency rule, and it is the thing P2's DAG actually protects. Worth reading as a design principle rather than three coincidences |
| **PF19** | **The metric set does not respect the create/read boundary B9 asserts.** `PDF_RENDER` is incurred by a `GET`, `POST /{id}/share` mints a link that renders forever, and `POST /{id}/revise` is a create that meters nothing. Worse, `/public/q/{token}` renders a PDF with **no JWT** — so no claims, no entitlements, and structurally nowhere to put a check. The application's only unauthenticated route is also its most expensive metered one | The create-endpoint rule (ER1) covers creates only, so every read-incurred metric sits outside it by construction. Either those metrics are soft COGS-protection (which the product spec half-says) or they need another mechanism — **a sub-project 10 decision, flagged now so it is not discovered by a bill**. The public route is a rate-limiting problem, and this is the second independent argument for pulling backlog item 3 forward |
| **PF17** | **`BYPASSRLS` cannot be constrained by PF14.** The outbox relay was designed to read across tenants via a `relay_app` role holding `BYPASSRLS` — which is a **role attribute, not a table permission**: it applies to every table in the database and it *overrides* `FORCE`. So the one role in the system whose job is to bypass tenant isolation was also the one role PF14's fix could not reach, and its containment rested on a GRANT list never widening | Replaced by two role-scoped permissive policies on the outbox table alone (outbox LLD revision 4, OF6), so the exemption is scoped by construction. Verify first that Postgres OR-s permissive policies as this assumes — outbox LLD Appendix B item 7 |
---
