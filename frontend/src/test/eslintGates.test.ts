...
describe("R67: only 'zod/mini' may be imported, never bare 'zod'", () => {
  it('rejects a bare zod import in a feature', async () => {
    const { ids } = await lintOnce('src/features/auth/x.ts', "import { z } from 'zod';\nexport const s = z;\n");
    expect(ids).toContain('no-restricted-imports');
  });
